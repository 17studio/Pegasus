...
class CThostFtdcMdSpiImpl : public CThostFtdcMdSpi {
 public:
  ...
  virtual void OnFrontDisconnected(int nReason) override {
    callback_.disconnected(nReason);
  };

  ///心跳超时警告。当长时间未收到报文时，该方法被调用。
  ///@param nTimeLapse 距离上次接收报文的时间
  virtual void OnHeartBeatWarning(int nTimeLapse) override {
    callback_.heat_beat_warning(nTimeLapse);
  };
  ...
  virtual void OnRtnForQuoteRsp(
    ...
    callback_.quote(pForQuoteRsp);
  };

  ///完成各项回掉的设置
  void SetOnFrontConnected(ctp_md_front_connected_cb cb) {
    callback_.connected = cb;
  }
  void SetOnFrontDisconnected(ctp_md_front_disconnected_cb cb) {
    callback_.disconnected = cb;
  }
  void SetOnHeartBeatWarning(ctp_md_heart_beat_warning_cb cb) {
    callback_.heat_beat_warning = cb;
  }
  void SetOnRspUserLogin(ctp_md_user_login_rsp_cb cb) { callback_.login = cb; }
  void SetOnRspUserLogout(ctp_md_user_logout_rsp_cb cb) {
    callback_.logout = cb;
  }
  void SetOnRspError(ctp_md_error_rsp_cb cb) { callback_.error = cb; }
  void SetOnRspSubMarketData(ctp_md_sub_market_data_rsp_cb cb) {
    callback_.sub_market = cb;
  }
  void SetOnRspUnSubMarketData(ctp_md_unsub_market_data_rsp_cb cb) {
    callback_.unsub_market = cb;
  }
  void SetOnRspSubForQuoteRsp(ctp_md_sub_quote_rsp_cb cb) {
    callback_.sub_quote = cb;
  }
  void SetOnRspUnSubForQuoteRsp(ctp_md_unsub_quote_rsp_cb cb) {
    callback_.unsub_quote = cb;
  }
  void SetOnRtnDepthMarketData(ctp_md_depth_market_data_cb cb) {
    callback_.market_data = cb;
  }
...
void ctp_mdspi_set_unsub_market_data_rsp_cb(
    ctp_md_spi_t* spi,
